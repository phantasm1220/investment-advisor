...
class WatchEntry:
    ...
    sector:          str      # セクター
    analysis_memo:   dict = field(default_factory=dict)  # 分析メモ
...
class BuyWatchlist:
    ...
    def fill_missing_prices(self) -> int:
        """
        registered_price が 0 の手動登録銘柄の株価をyfinanceで補完する。
        スキャン実行時に自動で呼ばれる。
        Returns: 補完した件数
        """
        import yfinance as yf
        ...
        for entry in self._entries:
            if entry.registered_price == 0:
                try:
                    sym  = entry.ticker if entry.ticker.endswith(".T") else f"{entry.ticker}.T"
                    hist = yf.Ticker(sym).history(period="1d")
                    if not hist.empty:
                        price = round(float(hist["Close"].iloc[-1]), 1)
                        if price > 0:
                            entry.registered_price = price
                            changed = True
                            filled += 1
                            logger.info(
                                f"[Watchlist] 株価補完: {entry.ticker} → ¥{price}"
                            )
                except Exception as e:
                    logger.debug(f"[Watchlist] 株価補完失敗 {entry.ticker}: {e}")
        if changed:
            self._save()
        return filled
    ...
    def register_buy_decisions(
        ...
        for dec in decisions:
            ...
            # 各エージェントの判断・確信度を収集
            agents_summary = {}
            for sig in (dec.agent_signals or []):
                agents_summary[sig.agent_name] = {
                    "verdict":    sig.verdict.value,
                    "confidence": round(sig.confidence, 2),
                }

            # マネージャーの判断理由を100文字に要約（FinalDecisionのフィールドから取得）
            rationale = (dec.rationale or "")[:100]
            conflict  = (dec.conflict_note or "")[:50]
            # catalyst は agent_signals の raw_scores から取得
            catalyst = ""
            for sig in (dec.agent_signals or []):
                if sig.agent_name == "manager":
                    catalyst = str(sig.raw_scores.get("catalyst", ""))[:50]
                    break

            memo = {
                "agents":               agents_summary,
                "composite_confidence": round(dec.composite_confidence, 3),
                "registered_verdict":   dec.verdict.value,
                "rationale":            rationale,
                "catalyst":             catalyst,
                "conflict":             conflict,
            }
        ...
        try:
            ...
            self._entries = [
                    WatchEntry(
                        ...
                        registered_price= float(e.get("registered_price", 0)),
                        verdict         = e.get("verdict", "BUY"),
                        confidence      = float(e.get("confidence", 0.7)),
                        sector          = e.get("sector", "不明"),
                        analysis_memo   = e.get("analysis_memo", {}),
                    )
