...
class MarketDataFetcher:
    ...
    def get_top_volume_stocks(self, top_n: int = 100) -> list[StockInfo]:
        ...
        # 1. 株価100円以下を除外
        stocks = [s for s in stocks if s.current_price > 100]

        # 2. ETFを除外（銘柄コードのパターンで判定）
        #    東証ETFは 1300〜1999 番台が主体
        #    REITは 3000〜3999 番台にも一部存在するが個別株と混在するため除外しない
        #    明確なETFコード帯: 1300〜1999
        def _is_etf(code: str) -> bool:
            c_ = code.replace(".T", "")
            # 4桁コードのETF帯（1300〜1999）
            if c_.isdigit() and 1300 <= int(c_) <= 1999:
                return True
            # JPXリストの市場区分でETF/ETNと判定
            if self._listing_cache is not None:
                try:
                    row = self._listing_cache[self._listing_cache["コード"] == c_]
                    if not row.empty:
                        market = str(row.iloc[0].get("市場・商品区分", ""))
                        if "ETF" in market or "ETN" in market or "REIT" in market:
                            return True
                except Exception:
                    pass
            return False
        ...
        logger.info(
            f"フィルタ後: {after}件 "
            f"（除外: 株価100円以下+ETF = {before - after}件）"
        )

        # ── 出来高順にソートして上位 top_n 件を返す ──────────
    ...
    def get_stock_info(self, code: str) -> Optional[StockInfo]:
        ...
        try:
            ...
            # JPXリストの日本語名を優先（yfinanceは英語名が多い）
            from utils.name_resolver import get_jp_name as _gjn
            _yf_name = info.get("longName") or info.get("shortName") or code
            name     = _gjn(code.replace(".T",""), self._listing_cache) or _yf_name
