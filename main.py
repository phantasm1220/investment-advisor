...
# yfinance TzCacheのパスをtmpに設定（GitHub Actions環境での権限エラー回避）
try:
    import yfinance as _yf
    _yf.set_tz_cache_location(_os.path.join(tempfile.gettempdir(), "py-yfinance"))
except Exception:
    pass
import logging
import os
...
def run_single(args):
    ...
    if args.etf:
        ...
        result  = scanner.analyze_single(args.ticker, dry_run=False)
        if result is None:
            sys.exit(1)
    ...
    decision = InvestmentOrchestrator(
        discord_webhook_url=DISCORD_WEBHOOK_URL or None
          ...
          dry_run=False, stock_name=stock_name)

    from core.signal import Verdict
    # 全判定（BUY/HOLD/SELL問わず）分析完了は exit(0)
    # GitHub Actionsで exit(1) はエラー扱いになるため常に 0 を返す
    sys.exit(0)


def run_news_pick(args):
    """翌日注目銘柄ニュースピックをDiscordに送信"""
    from utils.news_picker import send_to_discord
    logger.info("=== ニュースピック開始 ===")
    ok = send_to_discord(DISCORD_WEBHOOK_URL or "")
    if not ok:
        logger.warning("ニュースピックの送信に失敗しました")
    logger.info("=== ニュースピック完了 ===")
...
def run_scan(args):
    ...
    MarketScanner(discord_webhook_url=DISCORD_WEBHOOK_URL or None).run_scan(
        top_n=args.top_n, dry_run=False, test_mode=args.test,
    )
...
def main():
    ...
    # news_pick サブコマンド
    sub.add_parser("news_pick", help="翌日注目銘柄ニュースピックをDiscordに送信")
    ...
    {"single": run_single, "scan": run_scan, "etf": run_etf,
     ...
     "news_pick": run_news_pick}[args.mode](args)
